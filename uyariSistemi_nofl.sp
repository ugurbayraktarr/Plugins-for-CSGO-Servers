		...
		if(CanUserTarget(client, Player))
		{
			if(GetTime() > uyariBekleme[Player] + 10)
			{
				if (AreClientCookiesCached(client))
				{
					...
					//uyarilar[Player]++;
					PrintToChatAll(" \x02[DrK # GaminG] \x04%s\x05 adlı oyuncunun\x02 %d\x05 uyarısı oldu!", nick, cookieValue);
					...
					if(cookieValue == 1)
					{
						...
						SilenceTimers[Player] = CreateTimer(600.0, SilenceKaldir, userId);
						PrintToChatAll(" \x02[DrK # GaminG] \x04%N\x05 10 dk \x0CSilence aldı!", Player);
					}
					if(cookieValue == 2)
					{
						//silenceKapanmaZamani[Player] = GetTime() + 300;
						ServerCommand("sm_silence #%d", userId);
						SilenceTimers[Player] = CreateTimer(1200.0, SilenceKaldir, userId);
						PrintToChatAll(" \x02[DrK # GaminG] \x04%N\x05 20 dk \x0CSilence aldı!", Player);
					}
					...
					if(cookieValue >= 3)
					{
						...
						PrintToChatAll(" \x02[DrK # GaminG] \x04%s \x05adlı oyuncunun 3 uyarısı olduğu için banlandı", nick);
						BanClient(Player, 20, BANFLAG_AUTO, "3 uyarı aldınız ve 5 dk banlandınız.", "3 uyarı aldınız ve 20 dk banlandınız.",  "sm_ban", client);
					}
				}
			}
			else
			{
				PrintToChatAll(" \x02[DrK # GaminG] \x04%s \x05adlı oyuncuya uyarı vermek için \x04%d saniye \x05bekleyiniz.", nick, 10 - (GetTime() - uyariBekleme[Player]));
			}
		}
		else
		{
...
public Action SilenceKaldir(Handle timer, any userId)
{
	...
	PrintToChatAll(" \x02[DrK # GaminG] \x10%N \x04nickli oyuncunun silence cezası bitti!", client);
...
public Action:Command_Uyarim(client, args)
{
	...
	PrintToChat(client, " \x02[DrK # GaminG] \x04 %d\x05 uyarınız var!", cookieValue);
