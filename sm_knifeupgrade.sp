...
{      
	...
	if (knife_choice[client] > 0) 
	{
		...
		if (!CheckCommandAccess(client, "sm_knifeupgrade_shadow", 0, true) && knife_choice[client] == 10) {
			knife_choice[client] = 0;
			CPrintToChat(client, "%t","No Longer Has Access to Current Knife");
		}
		
		if (!CheckCommandAccess(client, "sm_knifeupgrade_bowie", 0, true) && knife_choice[client] == 11) {
			knife_choice[client] = 0;
			CPrintToChat(client, "%t","No Longer Has Access to Current Knife");
		}

		if (!CheckCommandAccess(client, "sm_knifeupgrade_golden", 0, true) && knife_choice[client] == 12) {
			knife_choice[client] = 0;
			CPrintToChat(client, "%t","No Longer Has Access to Current Knife");
		}
	}
	...
	switch(knife_choice[client]) {
		...
		case 10:iItem = GivePlayerItem(client, "weapon_knife_push");
		case 11:iItem = GivePlayerItem(client, "weapon_knife_survival_bowie");
		case 12:iItem = GivePlayerItem(client, "weapon_knifegg");
		default: return;
	}
	if (iItem > 0) 
...
SetShadow(client)
{
	...
	if (CheckCommandAccess(client, "sm_knifeupgrade_shadow", 0, true))
	{
		...
		CreateTimer(g_fDelayedEquipTimer, CheckKnife, GetClientSerial(client), TIMER_FLAG_NO_MAPCHANGE);
		if (g_bKnifeChosenMessage)
			CPrintToChat(client, " {GREEN}[DrK # GaminG] {RED}Shadow bicagini sectiniz!");
	} else {
		CPrintToChat(client, "%t", "Knife Access Denied Message");
	}
}
...
SetBowie(client)
{
	...
	if (CheckCommandAccess(client, "sm_knifeupgrade_bowie", 0, true))
	{
		...
		CreateTimer(g_fDelayedEquipTimer, CheckKnife, GetClientSerial(client), TIMER_FLAG_NO_MAPCHANGE);
		if (g_bKnifeChosenMessage)
			CPrintToChat(client, " {GREEN}[DrK # GaminG] {RED}Bowie bicagini sectiniz!");
	} else {
		CPrintToChat(client, "%t", "Knife Access Denied Message");
	}
}
...
public Action:ShowKnifeMenu(client)
{
	...
	//Shadow
	if (CheckCommandAccess(client, "sm_knifeupgrade_shadow", 0, true)) {
		AddMenuItem(menu, "option10", Shadow);
	} else {
		if(!g_bHideRestricted)
			AddMenuItem(menu, "option10", Shadow,ITEMDRAW_DISABLED);
	}
	//Bowie
	if (CheckCommandAccess(client, "sm_knifeupgrade_bowie", 0, true)) {
		AddMenuItem(menu, "option11", Bowie);
	} else {
		if(!g_bHideRestricted)
			AddMenuItem(menu, "option11", Bowie,ITEMDRAW_DISABLED);
	}
	//Golden
	if (CheckCommandAccess(client, "sm_knifeupgrade_golden", 0, true) && g_bEnableGoldenKnife) {
		AddMenuItem(menu, "option12", Golden);
	} else {
		if(!g_bHideRestricted) 
		{
			if (g_bEnableGoldenKnife)
				AddMenuItem(menu, "option12", Golden,ITEMDRAW_DISABLED);
		}
	}
	//Default
	AddMenuItem(menu, "option13", Default);
...
public ShowKnifeMenuHandler(Handle:menu, MenuAction:action, client, itemNum)
{
	switch(action){
		case MenuAction_Select: 
		{
			...
			//Shadow
			else if (strcmp(info,"option10") == 0) SetShadow(client);
			//Shadow
			else if (strcmp(info,"option11") == 0) SetBowie(client);
			//Golden
			else if (strcmp(info,"option12") == 0) SetGolden(client);
			//Default
			else if (strcmp(info,"option13") == 0) SetDefault(client);
		}
