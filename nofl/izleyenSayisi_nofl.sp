...
public Action izleyenIptalMenu(int client, int args)
{
	...
	SetMenuTitle(g_izleyenMenu, "[ ☜ NoFL ☞ ] İzleyen Sayısı Gösterme Sistemi");
	char Item[128];
	if(cookievalue == 0)
	{
...
public Action:SurekliTimer(Handle timer)
{
	...
	for(i=1;i<=MaxClients;i++)
	{
		...
		if(IsPlayerAlive(i))
		{
			if(izleyenSayisi[i] > 30) // Yaşayanların görme sayısı
				if((GetIntCookie(i, g_izleyenCookie)) == 0)
					PrintHintText(i, "<font color='#FF0000'>NoFL İzleme Sistemi\n<font color='#00FFFF'>Şuan <font color='#FF00FF'>%d <font color='#00FFFF'>kişi sizi izliyor.", izleyenSayisi[i]);
		}
		else
		{
			//izlenen = GetEntPropEnt(i, Prop_Send, "m_hObserverTarget");
