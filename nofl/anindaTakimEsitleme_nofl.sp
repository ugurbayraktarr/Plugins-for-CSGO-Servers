...
public Action TakimlariEsitle(Handle timer)
{
		...
		for(k=0; k<aradakiFark; k+=2)
		{
			...
			if(TakimdakiOyunculariSay(2) - TakimdakiOyunculariSay(3) > 1 || TakimdakiOyunculariSay(2) - TakimdakiOyunculariSay(3) < -1)
			{
				...
				if(cokOlanTakim == 2)
				{
					...
					PrintToChatAll(" \x02[ ☜ NoFL ☞ ] \x0CTakım eşitlemesi için \x10%N \x0CCT'ye atıldı.", takimdakiClientler[random]);
				}
				else
				{
					CS_SwitchTeam(takimdakiClientler[random], 2);
					CS_UpdateClientModel(takimdakiClientler[random]);
					PrintToChatAll(" \x02[ ☜ NoFL ☞ ] \x0CTakım eşitlemesi için \x10%N \x0CT'ye atıldı.", takimdakiClientler[random]);
				}
				esitlendi = true;
			}
		}
	...
	if(esitlendi)
		PrintToChatAll(" \x02[ ☜ NoFL ☞ ] \x10Takımlar eşitlenmiştir.");
}
