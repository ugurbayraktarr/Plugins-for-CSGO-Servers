...
public Action:OnSay( client, args )
{
	...
	if(kufurVar)
	{
		...
		PrintToChatAll(" \x02[ ☜ NoFL ☞ ] \x10%N \x04Küfür ettiği için silence aldı.", client);
		new userid = GetClientUserId(client);
		ServerCommand("sm_silence #%d", userid);
		PrintToChat(client, " \x10Küfür ettiğiniz için \x07sistem tarafından atıldınız. \x04(%s) ", gercekMesaj);
