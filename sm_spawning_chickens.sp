			...
			if(TR_DidHit(trace))
			{
				if(TR_GetEntityIndex(trace) == 0)
				{
					...
					if(IsValidEntity(chicken))
					{
						...
						DispatchKeyValue(chicken, "rendercolor", "255 0 0"); //Chickens model color (R, G, B)
						DispatchKeyValue(chicken, "modelscale", "5"); //Chickens model scale (0.5 smaller, 1.5 bigger chicken, min: 0.1, max: -)
