...
public Action:OnSay( client, args )
{
	...
	if(kufurVar)
	{
		...
		{
			PrintToChatAll(" \x02[%s] \x10%N \x04Küfür ettiği için silence aldı.", sPluginTagi, client);
			new userid = GetClientUserId(client);
			ServerCommand("sm_silence #%d", userid);
			PrintToChat(client, " \x10Küfür ettiğiniz için \x07sistem tarafından \x02silence \x07aldınız. \x04(%s) ", gercekMesaj);
		}
		else
			PrintToChat(client, "  \x02Sakin ol biraz bebeğim :)))");
