...
ArrayList ctSound;
ArrayList trSound;
new Handle:g_cVar_ClutchMuzikYolu = INVALID_HANDLE;
...
public void OnPluginStart()
{
	...
	g_cVar_ClutchMuzikYolu			=	CreateConVar("drk_cluth_muzikyolu", "sound/misc/drkclutch/ClutchMuzigi.mp3", "Clutch muzigi yolunu giriniz. (Ornek: sound/misc/drkclutch/ClutchMuzigi.mp3)");
	...
	new String:sClutchMuzikYolu[64];
	
	GetConVarString(g_cVar_ClutchMuzikYolu, sClutchMuzikYolu, sizeof(sClutchMuzikYolu));
	
	AddFileToDownloadsTable(sClutchMuzikYolu);
}
	...
	if((winner == CS_TEAM_CT && SamePath) || winner == CS_TEAM_T)
	{
		...
		else
		{
			if(!SamePath) 
			{
				...
				PrintToChatAll("{red}[%s] {default}TR_SOUNDS ERROR: Sounds not loaded.", sPluginTagi);
			}
			else
			{
				PrintToServer("[AbNeR RES] SOUNDS ERROR: Sounds not loaded.");
				PrintToChatAll("{red}[%s] {default}SOUNDS ERROR: Sounds not loaded.", sPluginTagi);
			}
			return;
		}
	}
...
public Action abnermenu(int client, int args)
{
	...
	SetMenuTitle(g_AbNeRMenu, "Round Sonu Sarkilari");
	char Item[128];
	if(cookievalue == 0)
	{
...
{	
		...
		for (int i = 1; i <= MaxClients; i++)
		{
			if(IsValidClient(i) && GetIntCookie(i, g_AbNeRCookie) == 0)
			{
				...
				else if(clutch)
				{
					...
					GetConVarString(g_cVar_ClutchMuzikYolu, sClutchMuzikYolu, sizeof(sClutchMuzikYolu));
					ClientCommand(i, "playgamesound Music.StopAllMusic");
					ClientCommand(i, "play \"*%s\"", sClutchMuzikYolu);
				}
			}
		}
	}
	...
	if(!clutch)
	{
		PrintToChatAll(" \x02[%s] \x0EŞuan Çalınan:\n \x10%s", sPluginTagi, sarkiIsmiYazdir);
	}
	else if(clutch)
	{
		decl String:clutchIsim[32];
		GetClientName(iClutch, clutchIsim, sizeof(clutchIsim));
		PrintToChatAll(" \x02[%s] \x04%s \x03, \x0C1 v %d \x10Clutch Atmıştır, \x04Tebrikler!", sPluginTagi, clutchIsim, clutchSayisi);
		PrintCenterTextAll("<font color='#FF0000'>[%s]</font> <font color='#00FF00'>%s</font>\n<font color='#ff7700'>1 v %d</font> <font color='#00FFFF'>Clutch atmıştır, Tebrikler!</font>",sPluginTagi, clutchIsim, clutchSayisi);
			
		PrintToChatAll(" \x02[%s] \x10Şuan Çalınan:\n \x04Clutch Müziği", sPluginTagi);
	}
}
