...
public OnPluginStart()
{
	...
	RegAdminCmd("sm_god", Command_godmode, ADMFLAG_CUSTOM1, "[SM] Usage: sm_god");
	RegAdminCmd("sm_buddha", Command_buddha, ADMFLAG_CUSTOM1, "[SM] Usage: sm_buddha");
...
{
	if (args == 0 || !CheckCommandAccess(client, "sm_ag_other", ADMFLAG_CUSTOM1, true))
	{
...
public Action:NoclipEvent(Handle:Timer, any:client)
{
	if (g_NoclipGod)
	{
		if(IsClientInGame(client) && IsPlayerAlive(client))
		{
			...
			if (movetype == MOVETYPE_NOCLIP && GetUserFlagBits(client) & ADMFLAG_CUSTOM1)
			{
				GiveGodmode(client);
