...
public Action:Command_Admins( client, args )
{
	...
	else
	{
		...
		if(ustYetkiliSayisi != 0)
		{
			...
			PrintToChat(client, " \x07Baş Yetkili: %s", bufferUstYetkili);
		}
		if(ozelSayisi != 0)
		{
			...
			PrintToChat(client, " \x0BSunucu Yardımcısı: \x10%s", bufferOzel);
		}
		if(yetkiliSayisi != 0)
		{
			...
			PrintToChat(client, " \x0EAdmin: %s", bufferYetkili);
		}
		if(adminSayisi != 0)
		{
			...
			PrintToChat(client, " \x04Stajyer Apex: %s", bufferAdmin);
		}
	}
