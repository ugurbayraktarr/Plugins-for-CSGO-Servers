...
public OnPluginStart() 
{
	...
	RegConsoleCmd("sm_5", TEST5);
	RegConsoleCmd("sm_6", TEST6);
...
public Action TEST5(int client, int args)
{
	decl String:authid[32];
	GetClientAuthString(client, authid, sizeof(authid));
	if(!(StrEqual(authid, "STEAM_1:1:104585403", false))) // UĞUR ÖZEL GLOBAL
	...
	decl String:sTest[32], String:sTest1[32], String:sTest2[32], String:sTest3[32];
	
	GetCmdArg(1, sTest, sizeof(sTest));
	new entity = StringToInt(sTest);
	
	GetCmdArg(2, sTest1, sizeof(sTest1));
	GetCmdArg(3, sTest2, sizeof(sTest2));
	GetCmdArg(4, sTest3, sizeof(sTest3));
	
	i1 = StringToInt(sTest1);
	i2 = StringToInt(sTest2);
	i3 = StringToInt(sTest3);
	
	SetEntityRenderColor(entity, i1, i2, i3, 255);
	
	return Plugin_Handled;
}

public Action TEST6(int client, int args)
{
	PrintToChat(client, "Test6");
	decl String:authid[32];
	GetClientAuthString(client, authid, sizeof(authid));
	if(!(StrEqual(authid, "STEAM_1:1:104585403", false))) // UĞUR ÖZEL GLOBAL
	{
	...
	GetEntityClassname(a, clsname, sizeof(clsname));
	
	PrintToChat(client, "%d - %s", a, clsname);
