...
public void OnPluginStart()
{
	...
	AddFileToDownloadsTable("sound/misc/drkclutch/Clutch Muzigi.mp3");
}
...
public Action msg(Handle timer, any client)
{
	...
	{
		CPrintToChat(client, "{default}{green}[DrK # GaminG] {default}%t", "JoinMsg");
	}
}
...
{	
	if(CSGO)
	{
		for (int i = 1; i <= MaxClients; i++)
		{
			if(IsValidClient(i) && GetIntCookie(i, g_AbNeRCookie) == 0)
			{
				...
				{
					ClientCommand(i, "playgamesound Music.StopAllMusic");
					ClientCommand(i, "play \"*misc/drkclutch/Clutch Muzigi.mp3\"");
				}
			}
		}
	}
	...
	if(!clutch)
	{
		CPrintToChatAll(" \x02[DrK # GaminG] \x10Şuan Çalınan:\n \x04%s", sarkiIsmiYazdir);
	}
	else if(clutch)
	{
		decl String:clutchIsim[32];
		GetClientName(iClutch, clutchIsim, sizeof(clutchIsim));
		CPrintToChatAll(" \x02[DrK # GaminG] \x04%s \x03, \x0C1 v %d \x10Clutch Atmıştır, \x04Tebrikler!", clutchIsim, clutchSayisi);
