...
public Action:Command_Admins( client, args )
{
	...
	else
	{
		...
		if(ustYetkiliSayisi != 0)
		{
			...
			PrintToChat(client, " \x07Üst Düzey Yetkili: %s", bufferUstYetkili);
		}
		if(ozelSayisi != 0)
		{
			...
			PrintToChat(client, " \x0BOzel Admin: \x10%s", bufferOzel);
		}
		if(yetkiliSayisi != 0)
		{
			...
			PrintToChat(client, " \x0EYetkili Admin: %s", bufferYetkili);
		}
		if(adminSayisi != 0)
		{
