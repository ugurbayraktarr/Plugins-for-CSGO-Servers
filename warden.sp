...
public OnPluginStart()
{
		...
		g_cVar_SpriteVMTLocation = CreateConVar("sm_warden_sprite_vmt", "materials/sprites/drkkomutculogoo.vmt", "Location of warden sprite .vmt file");
		g_cVar_SpriteVTFLocation = CreateConVar("sm_warden_sprite_vtf", "materials/sprites/drkkomutculogoo.vtf", "Location of warden sprite .vtf file");
...
public Action:HookPlayerChat(client, const String:command[], args)
{
		if(Warden == client && client != 0) // Check so the player typing is warden and also checking so the client isn't console!
		{
				...
				{
						PrintToChatAll(" \x04[-Komutçu-] \x10%N \x01: \x01%s", client, szText);
						return Plugin_Handled;
				}
		}
